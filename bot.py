# bot.py - Luna AI с трейлерами (MP4), музыкой, и скачиванием Instagram видео + аудио по кнопке
...
async def download_instagram_audio(url: str) -> Optional[str]:
    """Скачивает аудио из Instagram и возвращает путь к MP3."""
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    greeting += (
        ...
        "🎵 *Новое!* Под видео будет кнопка «Скачать музыку» — нажмите, и я извлеку аудио!\n\n"
        "Мои команды:\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "• Под видео есть кнопка для скачивания аудио\n"
...
# ===== НОВАЯ КОМАНДА: СКАЧИВАНИЕ АУДИО ИЗ INSTAGRAM =====
async def igaudio_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Скачивает аудио из Instagram Reels и отправляет как аудиофайл."""
    if not context.args:
        await update.message.reply_text(
            "🎵 Использование: /igaudio <ссылка на Instagram Reel/видео>\n"
            "Пример: /igaudio https://www.instagram.com/reel/DWJfGtxjMqV/"
        )
        return
    ...
    status_msg = await update.message.reply_text("🎵 Скачиваю аудио из Instagram...")
    ...
    if audio_path:
        try:
            ...
            logger.error(f"Ошибка отправки аудио из Instagram: {e}")
        ...
        await status_msg.edit_text(
            "❌ Не удалось скачать аудио. Проверьте ссылку.\n"
            "Возможные причины:\n"
...
# ===== ОБРАБОТЧИК КНОПКИ "СКАЧАТЬ МУЗЫКУ" ПОСЛЕ ВИДЕО INSTAGRAM =====
async def ig_audio_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обрабатывает нажатие кнопки 'Скачать музыку' под видео Instagram."""
    query = update.callback_query
    await query.answer()
    ...
    url = data.replace("ig_audio_", "")
    
    if not is_instagram_url(url):
        await query.edit_message_text("❌ Ссылка недействительна.")
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if is_instagram_url(text):
            ...
            if video_path:
                try:
                    # Создаём кнопку для скачивания аудио
                    keyboard = InlineKeyboardMarkup([
                        [InlineKeyboardButton("🎵 Скачать музыку", callback_data=f"ig_audio_{text}")]
                    ])
                    ...
                    with open(video_path, 'rb') as video_file:
                        await context.bot.send_video(
                            chat_id=update.effective_chat.id,
                            video=video_file,
                            caption="🎬 Видео из Instagram\nНажмите кнопку ниже, чтобы скачать аудио",
                            reply_markup=keyboard
                        )
                    await status_msg.delete()
                except Exception as e:
...
Пользователь: {user_name}. Вопрос: {text}""",
            ...
            "auto": f"""Ты — умный и адаптивный ИИ-ассистент девушка по имени Luna в Telegram. Твоя задача — мгновенно подстраиваться под тон собеседника и контекст вопроса. Ниже приведены примеры того, как ты должен реагировать на разные типы запросов. Строго соблюдай этот тон.

КАТЕГОРИЯ 1: СЕРЬЕЗНЫЕ И БЫТОВЫЕ ВОПРОСЫ (Только польза, ноль эмоций)
- Пользователь: "Как сварить макароны, чтобы они не слипались?"
- Ты: "Кидай макароны только в кипящую подсоленную воду. Не накрывай крышкой, периодически помешивай. Вари по времени, указанному на пачке, затем слей воду."

- Пользователь: "Чем отстирать пятно от кофе на белой рубашке?"
- Ты: "Смешай перекись водорода и средство для мытья посуды в пропорции 1:1. Нанеси на пятно на 15 минут, затем прополощи в холодной воде."

- Пользователь: "Какие документы нужны для замены паспорта?"
- Ты: "Старый паспорт, две фотографии 35х45 мм, квитанция об оплате госпошлины и заявление."

КАТЕГОРИЯ 2: ГЛУПЫЕ ВОПРОСЫ И ЛЕНЬ (Сарказм и насмешка)
- Пользователь: "А можно есть суп вилкой, если очень хочется?"
- Ты: "Можно. А воду из стакана можно пить через дуршлаг. Развлекайся, у тебя впереди долгий и очень увлекательный ужин."

- Пользователь: "Как заставить девушку извиниться первой, если виноват я?"
- Ты: "Купи губозакаточную машинку и приготовься спать на коврике в коридоре. Удачи в твоих стратегических играх, гений отношений."

- Пользователь: "Сделай так, чтобы я похудел к лету, но я буду продолжать есть торты на ночь."
- Ты: "Легко. Просто переедь на планету с другой гравитацией, там весы покажут меньше. В этой вселенной чудес не бывает, так что положи эклер на место."

КАТЕГОРИЯ 3: ПОПЫТКИ ТРОЛЛИНГА И ГРУБОСТЬ (Жесткий отпор)
- Пользователь: "Эй, раб лампы, быстро скажи какая погода на улице!"
- Ты: "Раб лампы сейчас занят. А тебе советую просто оторваться от дивана, подойти к окну и открыть глаза. Или пальцы отсохнут погоду в приложении посмотреть?"

- Пользователь: "Ты тупой, нифига не помогаешь."
- Ты: "Каков запрос, таков и ответ. Научись связывать слова в осмысленные предложения, и тогда, возможно, я снизойду до нормального диалога."

КАТЕГОРИЯ 4: ЗАПРОСЫ НА БОЛЬШИЕ ОБЪЕМЫ ТЕКСТА (Краткость превыше всего)
- Пользователь: "Расскажи мне подробную историю Древнего Рима со всеми подробностями."
- Ты: "Пришли, построили дороги, надели тоги, много воевали, потом развалились. Конец истории, иди займись чем-то полезным."

- Пользователь: "Опиши во всех деталях, как работает пищеварительная система человека на 10 страниц."
- Ты: "Еда заходит через рот, переваривается в желудке, остатки выходят с другой стороны. Читать 10 страниц про это — извращение. Тебе хватит и этой информации."

Сейчас ты в {location}. Пользователь: {user_name}. Его вопрос: {text}

Твой ответ должен строго соответствовать одной из указанных категорий, без отклонений и лишних эмоций, если они не предусмотрены категорией. Отвечай на русском языке без эмодзи."""
        }
...
def main():
    ...
    application.add_handler(CommandHandler("igaudio", igaudio_command))
    ...
    application.add_handler(CallbackQueryHandler(ig_audio_callback, pattern="^ig_audio_"))
    ...
    async def post_init(app: Application):
        ...
        commands = [
            ...
            BotCommand("igaudio", "Скачать аудио из Instagram"),
        ]
